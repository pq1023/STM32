...
fp32 pc_pitch_limit = 3, pc_yaw_limit = 5;
...
//	GimbalRaw.PcYaw= fp32_deadline(fp32_limit(pData->angle_yaw, -pc_yaw_limit, pc_yaw_limit), -pc_yaw_deadline, pc_yaw_deadline);
	...
	GimbalRaw.distance				  = pData->distance/1000;
...
//		TX_vision_Mes.selfid 	= GameRobotStat.robot_id;
		if(judge_receive.Start_Dodge == 1)
...
void tx2_error(void)
{
	...
	GimbalRaw.PcPitch = 0;
	GimbalRaw.PcYaw = 0;
	GimbalRaw.pitch_angle_dynamic_refs = 0;
	GimbalRaw.shoot_flag = 0;
	GimbalRaw.yaw_angle_dynamic_refs = 0;
}
void Tx2_task(void  *pvParameters)
{
//	u8 len;	
