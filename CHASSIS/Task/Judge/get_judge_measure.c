...
extern ext_game_robot_survivors_t          		GameRobotSurvivors;			//0x0003
...
extern ext_projectile_allowance_t    Bullet_Remaing;	//0x0208
extern ext_student_interactive_header_data_t  student_interactive;//0x0301	
extern ext_robot_command_t              robot_command;

//返回裁判系统接收端变量地址，通过指针方式获取原始数据
const ext_game_state_t *get_game_state_t(void)//比赛状态数据
{
	return &GameState;
}
...
const ext_game_robot_survivors_t *get_game_robot_survivors_t(void)//比赛机器人存活数据
{
...
const ext_game_robot_state_t *get_game_robot_state_t(void)//机器人状态数据
{
...
const ext_robot_command_t *get_Robot_Command_t(void)//小地图位置数据
{
	return &robot_command;
}

const ext_buff_musk_t *get_buff_musk_t(void)//机器人增益数据
{
	return &BuffMusk;
}
...
const ext_projectile_allowance_t *get_projectile_allowance_t(void)//允许发单量剩余数据
{
	return &Bullet_Remaing;
}
