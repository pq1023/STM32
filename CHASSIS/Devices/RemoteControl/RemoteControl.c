...
void USART2_IRQHandler(void)
{
    ...
    {
        USART_ReceiveData(USART2);//清空空闲挂起标志
    }
    else if (USART_GetITStatus(USART2, USART_IT_IDLE) != RESET)//空闲中断
    {
        ...
        if(DMA_GetCurrentMemoryTarget(DMA1_Stream5) == 0)		//大疆DMA1_Stream5
        {
            //重新设置DMA
            DMA_Cmd(DMA1_Stream5, DISABLE);
            this_time_rx_len = SBUS_RX_BUF_NUM - DMA_GetCurrDataCounter(DMA1_Stream5);
            DMA_SetCurrDataCounter(DMA1_Stream5, SBUS_RX_BUF_NUM);	//将 dma 内存指针重新定位到起始位置
            DMA1_Stream5->CR |= DMA_SxCR_CT;	//启用当前选定的内存为内存 1
            //清DMA中断标志
...
static void RC_MODE_CONTROL(RC_ctrl_t *rc_ctrl)
{
    ...
    if (rc_ctrl->rc.ch[0] == -660 && rc_ctrl->rc.ch[1] == -660 ) //拨杆内八软件强制复位
    {
